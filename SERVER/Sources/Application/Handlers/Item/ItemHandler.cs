...
namespace NRO_Server.Application.Handlers.Item
{
    public class ItemHandler
    {
		...
		private static void UsePhaohoa(Model.Character.Character character, Model.Item.Item item)
        {
            if (character.CharacterHandler.GetItemBagById(1036) == null) return;
            ...
            if (tile < 30)
            {
                var gold = ServerUtils.RandomNumber(500000,1000000);
                character.CharacterHandler.SendMessage(
            ...
            {
                var listitem = new List<short>() {17,18,19,20,381,382,383,384}; // ngoc rồng
                ...
                character.CharacterHandler.AddItemToBag(true, itemAdd, "HQT");
                character.CharacterHandler.SendMessage(
			...
			//else if (tile < 50)
            //{
            //    var listitem = new List<short>() {668}; // Hộp quà tết
            //    var itemrand = listitem[ServerUtils.RandomNumber(listitem.Count)];
            //    var itemAdd = ItemCache.GetItemDefault(itemrand);
            //    var temp = ItemCache.ItemTemplate(itemAdd.Id);
            //    character.CharacterHandler.AddItemToBag(true, itemAdd, "HQT");
			...
			character.DiemSuKien += 1;
			 character.CharacterHandler.SendMessage(
                Service.ServerMessage(string.Format("Bạn nhận được 1 điểm sự kiện 8/3")));
            character.CharacterHandler.SendMessage(Service.SendBag(character));
			
        }
		private static void UseHopcs(Model.Character.Character character, Model.Item.Item item)
        {
            if (character.CharacterHandler.GetItemBagById(398) == null) return;
            character.CharacterHandler.RemoveItemBagByIndex(item.IndexUI, 1, reason:"Quà 8/3");
            var tile = ServerUtils.RandomNumber(100);
            //50 % vàng, 20% nr, 20% item ngẫu nhiên, 8% cải trang, 2% cải trang v.v
            if (tile < 30)
            {
            ...
            //else if (tile < 40)
            //{
            //    var listitem = new List<short>() {14,15,702,703,704,1093,1094,921}; // ngoc rồng
            //    var itemrand = listitem[ServerUtils.RandomNumber(listitem.Count)];
            //    var itemAdd = ItemCache.GetItemDefault(itemrand);
            //    var temp = ItemCache.ItemTemplate(itemAdd.Id);
            //    character.CharacterHandler.AddItemToBag(true, itemAdd, "Quà 8/3");
            //    character.CharacterHandler.SendMessage(
            //        Service.ServerMessage(string.Format(TextServer.gI().ADD_ITEM,
            //            $"{temp.Name}")));
            //}
			//else if (tile < 5)
            //{
            //    var listitem = new List<short>() {915, 758}; // ngoc rồng
            //    var itemrand = listitem[ServerUtils.RandomNumber(listitem.Count)];
            //    var itemAdd = ItemCache.GetItemDefault(itemrand);
            //    var temp = ItemCache.ItemTemplate(itemAdd.Id);
            //    character.CharacterHandler.AddItemToBag(true, itemAdd, "Quà 8/3");
            ...
            {
                var listitem = new List<short>() {1102,1101,1099,1106,1080,1083,1084,1071,1072,999,954}; // item ngẫu nhiên
                var itemrand = listitem[ServerUtils.RandomNumber(listitem.Count)];
                var itemAdd = ItemCache.GetItemDefault(itemrand);
        ...
        private static void UseCapsuleHalloween(Model.Character.Character character, Model.Item.Item item)
        {
            try
            {
                ...
                var randomPercent = ServerUtils.RandomNumber(0, 30);
                if (randomPercent <= 70)
                {
                // - đồ kích hoạt 0.3%
        ...
        private static void UseCapsuleTrungThu(Model.Character.Character character, Model.Item.Item item)
        {
            ...
            {
                var listitem = new List<short>() {16,16,17,17,17,18,18,18,18,19,19,19,19,19,20,20,20,20,20,20}; // ngọc rồng
                var itemrand = listitem[ServerUtils.RandomNumber(listitem.Count)];
                var itemAdd = ItemCache.GetItemDefault(itemrand);
                var temp = ItemCache.ItemTemplate(itemAdd.Id);
        ...
        private static void UseDragonBall(Model.Character.Character character, Model.Item.Item item)
        {
            ...
            // Gọi rồng thần
            MapManager.delayCallDragon = timeServer + 1000;
        ...
        private static void UsePetItem(Model.Character.Character character, Model.Item.Item item)
        {
            ...
            {
                character.CharacterHandler.SendMessage(Service.ServerMessage(string.Format("Linh thú đã bị lỗi, Dev Khánh Lỏ đang code")));
                return;
            }
